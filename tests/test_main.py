...
@pytest.mark.asyncio
async def test_handle_services_sync_success(initialized_db, mock_config):
    import main
    ...
    try:
        with (
            ...
            await main.handle_services_sync(mock_status_badge, mock_sync_label)
            assert mock_sync_label.text == "Sync complete"
            mock_refresh.assert_called_once()
    finally:
        main.config = original_config
...
@pytest.mark.asyncio
async def test_handle_services_sync_disabled(initialized_db, mock_config):
    import main

    original_config = main.config
    original_state = main.state
    main.config = mock_config
    main.state = SharedTestState(environment="staging")
    mock_status_badge, mock_sync_label = _make_mock_badges()

    try:
        with patch("main.ui.notify"):
            await main.handle_services_sync(mock_status_badge, mock_sync_label)
            assert "Sync disabled" in mock_sync_label.text
    finally:
        main.config = original_config
        main.state = original_state
...
@pytest.mark.asyncio
async def test_handle_services_sync_unauthorized(initialized_db, mock_config):
    import main
    ...
    try:
        with (
            ...
            mock_manager.sync_services = AsyncMock(side_effect=exc)
            mock_build.return_value = mock_api
            with patch("main.SyncManager", return_value=mock_manager):
                await main.handle_services_sync(mock_status_badge, mock_sync_label)
...
@pytest.mark.asyncio
async def test_handle_templates_sync_success(initialized_db, mock_config):
    import main
    ...
    try:
        with patch.object(main, "refresh_status_badge", new_callable=AsyncMock):
            await main.handle_templates_sync(mock_status_badge, mock_sync_label)
            assert mock_sync_label.text == "Sync complete"
    finally:
        main.config = original_config
        main.state = original_state
...
@pytest.mark.asyncio
async def test_handle_templates_sync_disabled(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_templates_sync_unauthorized(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_api_keys_sync_success(initialized_db, mock_config):
    import main
    ...
    try:
        with patch.object(main, "refresh_status_badge", new_callable=AsyncMock):
            await main.handle_api_keys_sync(mock_status_badge, mock_sync_label)
            assert mock_sync_label.text == "Sync complete"
    finally:
        main.config = original_config
        main.state = original_state
...
@pytest.mark.asyncio
async def test_handle_api_keys_sync_disabled(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_api_keys_sync_unauthorized(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_sms_senders_sync_success(initialized_db, mock_config):
    import main
    ...
    try:
        with patch.object(main, "refresh_status_badge", new_callable=AsyncMock):
            await main.handle_sms_senders_sync(mock_status_badge, mock_sync_label)
            assert mock_sync_label.text == "Sync complete"
    finally:
        main.config = original_config
        main.state = original_state
...
@pytest.mark.asyncio
async def test_handle_sms_senders_sync_disabled(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_sms_senders_sync_unauthorized(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_users_sync_success(initialized_db, mock_config):
    import main
    ...
    try:
        with patch.object(main, "refresh_status_badge", new_callable=AsyncMock):
            await main.handle_users_sync(mock_status_badge, mock_sync_label)
            assert mock_sync_label.text == "Sync complete"
    finally:
        main.config = original_config
        main.state = original_state
...
@pytest.mark.asyncio
async def test_handle_users_sync_disabled(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_users_sync_unauthorized(initialized_db, mock_config):
    import main
    ...
    try:
        with (
            ...
            mock_api = AsyncMock()
            mock_manager = AsyncMock()
            mock_manager.sync_users = AsyncMock(side_effect=exc)
            mock_build.return_value = mock_api
            with patch("main.SyncManager", return_value=mock_manager):
                await main.handle_users_sync(mock_status_badge, mock_sync_label)
                assert "Unauthorized" in mock_sync_label.text
...
@pytest.mark.asyncio
async def test_handle_provider_details_sync_success(initialized_db, mock_config):
    import main
    ...
    try:
        with patch.object(main, "refresh_status_badge", new_callable=AsyncMock):
            await main.handle_provider_details_sync(mock_status_badge, mock_sync_label)
            assert mock_sync_label.text == "Sync complete"
    finally:
        main.config = original_config
        main.state = original_state
...
@pytest.mark.asyncio
async def test_handle_provider_details_sync_disabled(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_provider_details_sync_unauthorized(initialized_db, mock_config):
    import main
    ...
    try:
        with (
            ...
            mock_api = AsyncMock()
            mock_manager = AsyncMock()
            mock_manager.sync_provider_details = AsyncMock(side_effect=exc)
            mock_build.return_value = mock_api
            with patch("main.SyncManager", return_value=mock_manager):
                await main.handle_provider_details_sync(
...
@pytest.mark.asyncio
async def test_handle_communication_items_sync_success(initialized_db, mock_config):
    import main
    ...
    try:
        with patch.object(main, "refresh_status_badge", new_callable=AsyncMock):
            await main.handle_communication_items_sync(
                mock_status_badge, mock_sync_label
            )
            assert mock_sync_label.text == "Sync complete"
    finally:
...
@pytest.mark.asyncio
async def test_handle_communication_items_sync_disabled(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_communication_items_sync_unauthorized(
    initialized_db, mock_config
):
    import main
    ...
    try:
        with (
            ...
            mock_api = AsyncMock()
            mock_manager = AsyncMock()
            mock_manager.sync_communication_items = AsyncMock(side_effect=exc)
            mock_build.return_value = mock_api
            with patch("main.SyncManager", return_value=mock_manager):
                await main.handle_communication_items_sync(
                    mock_status_badge, mock_sync_label
...
@pytest.mark.asyncio
async def test_shutdown(initialized_db):
    from nicegui import app as nicegui_app
...
# ===================================================================
# Category 4: Individual sync handler auth_missing branches
# (lines 258, 285, 313, 341, 369, 395, 421)
# ===================================================================


@pytest.mark.asyncio
async def test_handle_services_sync_auth_missing(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_templates_sync_auth_missing(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_api_keys_sync_auth_missing(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_sms_senders_sync_auth_missing(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_users_sync_auth_missing(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_provider_details_sync_auth_missing(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_communication_items_sync_auth_missing(
    initialized_db, mock_config
):
    import main
...
@pytest.mark.asyncio
async def test_handle_services_sync_reraises_non_401(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_templates_sync_reraises_non_401(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_api_keys_sync_reraises_non_401(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_sms_senders_sync_reraises_non_401(initialized_db, mock_config):
    import main
    ...
    try:
        with (
            ...
            with patch("main.SyncManager", return_value=mock_manager):
                with pytest.raises(httpx.HTTPStatusError):
                    await main.handle_sms_senders_sync(
                        mock_status_badge, mock_sync_label
                    )
    finally:
        main.config = original_config
...
@pytest.mark.asyncio
async def test_handle_users_sync_reraises_non_401(initialized_db, mock_config):
    import main
...
@pytest.mark.asyncio
async def test_handle_provider_details_sync_reraises_non_401(
    initialized_db, mock_config
):
    import main
...
@pytest.mark.asyncio
async def test_handle_communication_items_sync_reraises_non_401(
    initialized_db, mock_config
):
    import main
