...
- **`app/ui/callback_helpers.py`** — Pure helpers for service callback validation, payload building, API error normalization, and status-control state. Imports no NiceGUI, so it is directly unit-testable. Mirrors notification-api's callback type, channel, and completed-notification-status constants.
